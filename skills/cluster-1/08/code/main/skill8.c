...
static void test_alpha_display()
{
  ...
  while (1)
  {
    ...
    if (ret == ESP_OK)
    {
      displaybuffer[0] = ASCII_to_Alphanum(str[0]); //
      displaybuffer[1] = ASCII_to_Alphanum(str[1]); //
      displaybuffer[2] = ASCII_to_Alphanum(str[2]); //
      displaybuffer[3] = ASCII_to_Alphanum(str[3]); //
      printf("- wrote: %s \n\n", str);
    }
  }
}
