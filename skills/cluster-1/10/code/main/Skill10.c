...
static int global_count = 16;
// 0 indicates up and 1 indicates down.
...
{
  displaybuffer[0] = 0b1111111111111111; // 0
...
{
  //count up or down and output to LEDs
  while (1)
  {
    //decide counter
    if (global_count == 0)
    ...
    if (direction == 0)
    {
      global_count = (global_count + 1) % 16;
    }
    else if (direction == 1)
    {
      global_count = (global_count - 1) % 16;
    ...
    //print counter
    gpio_set_level(LED0, (global_count % 2));
    gpio_set_level(LED1, ((global_count / 2) % 2));
    gpio_set_level(LED2, ((global_count / 4) % 2));
    gpio_set_level(LED3, ((global_count / 8) % 2));
...
void app_main()
{
  ...
  xTaskCreate(button_press, "button_press", 4096, NULL, 5, NULL);
  xTaskCreate(print_state, "print_state", 4096, NULL, 9, NULL);
  xTaskCreate(counter, "counter", 4096, NULL, 7, NULL);
}
